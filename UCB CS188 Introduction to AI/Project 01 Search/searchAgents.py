...
class CornerState:
    def __init__(self, pac_pos, visited=[False, False, False, False]):
        """
        pac_pos: the position of the current Pacman
        visited: whether four corner has been visited
        """
...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState: pacman.GameState):
        ...
        # self.starting_corner_state = CornerState(self.startingPosition)
    ...
    def isGoalState(self, state: Any):
        ...
        #     if self.corners[i] == state.pac_pos:
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            if hitsWall is False:
                boolean = []
                for i in range(len(self.corners)):
                    value = ((nextx, nexty) == self.corners[i]) or state[i + 1]
    ...
    for i in range(foodGrid.height):
        for j in range(foodGrid.width):
            if foodGrid[j][i]:
                heuristic_value = max(
                    heuristic_value,
                    mazeDistance((j, i), position, NewState(state, problem.walls)),
                )
    return heuristic_value
