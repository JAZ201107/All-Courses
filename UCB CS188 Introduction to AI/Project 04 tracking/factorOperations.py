...
def joinFactors(factors: List[Factor]):
    ...
    # Ensure conditioned variables are not also unconditioned
    for factor in factors:
        conditioned.difference_update(factor.unconditionedVariables())

    # Determine the variable domain dictionary, assuming it's the same across all factors
    variableDomainsDict = list(factors)[0].variableDomainsDict()

    # Create a new factor with the identified unconditioned and conditioned variables
    new_factor = Factor(unconditioned, conditioned, variableDomainsDict)

    # Iterate over all possible assignments and compute the product of probabilities
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        new_unconditioned = set(unconditioned) - {eliminationVariable}
        new_factor = Factor(new_unconditioned, conditioned, variableDomainsDict)

        # Sum probabilities over the elimination variable
        for assignment in new_factor.getAllPossibleAssignmentDicts():
            prob_sum = 0
            for elim_var_value in variableDomainsDict[eliminationVariable]:
                # Update the assignment to include the elimination variable
                extended_assignment = assignment.copy()
                extended_assignment[eliminationVariable] = elim_var_value
                prob_sum += factor.getProbability(extended_assignment)
