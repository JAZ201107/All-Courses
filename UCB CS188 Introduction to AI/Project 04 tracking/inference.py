...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[GHOST0] = [
        (i, j) for i in range(X_RANGE) for j in range(Y_RANGE)
    ]
    variableDomainsDict[GHOST1] = [
        (i, j) for i in range(X_RANGE) for j in range(Y_RANGE)
    ]

    variableDomainsDict[OBS0] = list(range(0, (X_RANGE + Y_RANGE - 1 + MAX_NOISE)))
    variableDomainsDict[OBS1] = list(range(0, (X_RANGE + Y_RANGE - 1 + MAX_NOISE)))
    "*** END YOUR CODE HERE ***"
...
def inferenceByEnumeration(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict):
    ...
    eliminationVariables = (
        bayesNet.variablesSet() - evidenceVariablesSet
    ) - queryVariablesSet
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(
        bayesNet: bn,
        ...
        if eliminationOrder is None:  # set an arbitrary elimination order if None given
            eliminationVariables = (
                bayesNet.variablesSet() - set(queryVariables) - set(evidenceDict.keys())
            )
        ...
        # Interleave join and eliminate operations for each variable in the elimination order
        for variable in eliminationOrder:
            # Join factors containing the variable
            currentFactorsList, joinedFactor = joinFactorsByVariable(
                currentFactorsList, variable
            )
            # Eliminate the variable if the resulting factor has more than one unconditioned variable
        ...
        # Normalize the final factor to get the conditional probability distribution
...
def sampleFromFactorRandomSource(randomSource=None):
    ...
    def sampleFromFactor(factor, conditionedAssignments=None):
        ...
        if conditionedAssignments is None and len(factor.conditionedVariables()) > 0:
            raise ValueError(
                "Conditioned assignments must be provided since \n"
            ...
            if not conditionedVariables.issuperset(set(factor.conditionedVariables())):
                raise ValueError(
                    "Factor's conditioned variables need to be a subset of the \n"
        ...
        assignmentDictProbabilities = [
            CPT.getProbability(assignmentDict) for assignmentDict in assignmentDicts
        ]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghost_position in self.allPositions:
            self.beliefs[ghost_position] *= self.getObservationProb(
                observation, pacman_position, ghost_position, jail_position
            )
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghost_position in self.allPositions:
            predictions[ghost_position] *= self.getObservationProb(
                observation, pacman_position, ghost_position, jail_position
            )
